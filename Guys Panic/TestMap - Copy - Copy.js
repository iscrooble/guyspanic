...
function spawnShockwave(xPos = player.x-cam.scrollX, yPos = player.y-cam.scrollY){
    var shockwave = postFxPlugin.add(cam, { waveRadius: 0 });
...
/*    var spawnArray = [];
    ...
    */
    ...
    bullInitPos = [Phaser.Math.Between(60,screenH-60),Phaser.Math.Between(screenW*3/4,screenW-40)]
...
//    layerRT.draw(layer,0,0);
    ...
    UICam.ignore([player,layerRT,rt,drawRT,finishedImage, gagFinishedImage, playerAfter1, playerAfter2,playerAfter3]);
...
//    dPress = keyD.isDown;
    ...
    if(bullComb != null){
        if(bullComb.x > borderWidth + cam.scrollX && !bullComb._hasHit && !currentScene.anims.paused){
            bullComb.x-=20;
            if(getTilesWithin(mapArray,bullComb.x,bullComb.y,20+19,8).includes(drawColor)){
                killPlayer(bullComb.x,bullComb.y);
            }
...
//                    addSparkles(1);
        ...
        if((percentageRevealed > 80 || cPress) && !bossKill){
...
//                enemy._state = "stop";
            ...
            bullEnemyTint = currentScene.add.sprite(bullBoss.x,bullBoss.y,'bullEnemy').play('bullDie').setAlpha(0);
//            bullEnemyTint.anims.paused = true;
            bullEnemyTint.depth = 2;
            bullEnemyTint.setTintFill(0xffffff);
            ...
            currentScene.time.delayedCall(3000, () =>
                {
                    bullEnemyTint.destroy();
                    addExplosions(bullBoss.x,bullBoss.y,20,10)
                    cam.shake()
                    UICam.shake();
...
function glowShine(xPos = player.x,yPos = player.y,delayT = 56, playSFX = false){
...
function glowShine2(xPos = player.x,yPos = player.y,delayT = 56){
...
function updateEmitter(emitter) {
    var x1 = screenW * FloatBetween(0.2, 0.8);
    var y1 = screenH * FloatBetween(0.2, 0.6);
//    emitter.x = x1;
//    emitter.y = y1;
    emitter.x = x1;
    emitter.y = y1;
...
function getTilesWithin(array,x,y,w,h){
...
function spawnItem(){
    if (Math.floor(Math.random()*spawnItemRNG)==0){
        ...
        while(getTilesWithin(mapArray,itemX,itemY,30,30).includes(edgeColor)
...
//            glowSprite.setVisible(true);
//            glowSprite.play({key: 'glowAnim'});
            glowShine(undefined,undefined,1500,true);
            playerAfter1.setAlpha(0);
            playerAfter2.setAlpha(0);
            playerAfter3.setAlpha(0);
            player.setVisible(false);
...
//                console.log(enemy.y)
                ...
                //ugh
                //map.getTilesWithin(Math.floor(enemy.x)-8,Math.floor(enemy.y-7),3,7,undefined, 'mainDraw')
...
*/
                ...
                enemy._enemyTileIndexL = getTilesWithin(mapArray,enemy.x-4,enemy.y,2,8);
                enemy._enemyTileIndexR = getTilesWithin(mapArray,enemy.x+4,enemy.y,2,8);
                enemy._enemyTileIndexU = getTilesWithin(mapArray,enemy.x,enemy.y-4,8,2);
                enemy._enemyTileIndexD = getTilesWithin(mapArray,enemy.x,enemy.y+4,8,2);
...
//                enemy._enemyTileIndexD = mapArray[enemy.x,enemy.y+1];
                ...
                //bull: 68 by 136
                //map.getTilesWithin(Math.floor(enemy.x)-24,Math.floor(enemy.y-34),1,68,undefined, 'mainDraw')
                //enemy.scale
                var xScale1 = enemy.scale*24
                var xScale2 = enemy.scale*30
                var yScale1 = enemy.scale*34
                var yScale2 = enemy.scale*25
                bossLeft = getTilesWithin(mapArray,enemy.x-xScale1,enemy.y,3,yScale1);
                bossRight = getTilesWithin(mapArray,enemy.x+xScale1,enemy.y,3,yScale1)
                bossUp = getTilesWithin(mapArray,enemy.x,enemy.y-yScale1,xScale1,3)
                bossDown = getTilesWithin(mapArray,enemy.x,enemy.y+yScale1,xScale1,3)

                //map.getTilesWithin(Math.floor(enemy.x)-30,Math.floor(enemy.y-25),1,50,undefined, 'mainDraw')
                bossLeftHit = getTilesWithin(mapArray,enemy.x-xScale2,enemy.y,3,yScale2)
                bossRightHit = getTilesWithin(mapArray,enemy.x+xScale2,enemy.y,3,yScale2)
                bossUpHit = getTilesWithin(mapArray,enemy.x,enemy.y-yScale2,xScale2,3)
                bossDownHit = getTilesWithin(mapArray,enemy.x,enemy.y+yScale2,xScale2,3)

                bossLeftShrink = getTilesWithin(mapArray,enemy.x-xScale1-6,enemy.y,3,yScale1);
                bossRightShrink = getTilesWithin(mapArray,enemy.x+xScale1+6,enemy.y,3,yScale1)
                bossUpShrink = getTilesWithin(mapArray,enemy.x,enemy.y-yScale1-6,xScale1,3)
                bossDownShrink = getTilesWithin(mapArray,enemy.x,enemy.y+yScale1+6,xScale1,3)
...
//                        console.log(getTilesWithin(mapArray,enemy.x-30*enemy.scale,enemy.y-20*enemy.scale,30*enemy.scale,50*enemy.scale))
                        if(!enemy.flipX && getTilesWithin(mapArray,enemy.x-30*enemy.scale,enemy.y-20*enemy.scale,30,50).includes(drawColor)){
                            killPlayer(enemy.x,enemy.y);
                            sfxSwingHit.play();
                        }else if(enemy.flipX && getTilesWithin(mapArray,enemy.x+30*enemy.scale,enemy.y-20*enemy.scale,30,50).includes(drawColor)){
                            killPlayer(enemy.x,enemy.y);
                            sfxSwingHit.play();
                        }
                    }
...
//                }
                ...
                if(hardMode){
                    ...
                    if(enemy._enemyTileIndexR.includes(edgeColor) || enemy._enemyTileIndexR.includes(borderColor)){
                        enemy._xVel *= -1;
                    }
                    if(enemy._enemyTileIndexU.includes(edgeColor) || enemy._enemyTileIndexU.includes(borderColor)){
                        enemy._yVel *= -1;
                    }
                    if(enemy._enemyTileIndexD.includes(edgeColor) || enemy._enemyTileIndexD.includes(borderColor)){
                        enemy._yVel *= -1;
                    }    
                }
...
//                    if(mapArray[enemy.x,enemy.y]==drawColor){
//                        killPlayer(enemy.x,enemy.y);
//                    }
...
//                    if(overlappingTiles.filter(tile => tile == drawColor).length != 0){
//                    }
                }
            }
        }
...
//                                    y: player.y ,
                duration: 3000,
                onUpdate: () => {
                    thundercloud.x += (player.x - thundercloud.x)* 0.15
                    thundercloud.y += (player.y - thundercloud.y-50)* 0.15
                }
            },
            {
                from: 0,
                ...
                repeat: 5,
                onStart: () => {
//                    thundercloud.play({key: 'thundercloud2',startFrame: (thundercloud.anims.currentFrame.index+3)})
//console.log(thundercloud.anims.currentFrame)
...
function spawnExplodeWave(xPos = player.x, yPos = player.y){
    ...
    currentScene.time.addEvent({
        ...
        callback: () => {
            ...
            for(i=0;i<8;i++){
                ...
                addExplosions(player.x + xDel,player.y + yDel,5,Math.floor((explodeDist)/20))
            }
...
flavorText.setTintFill(scoreTint);
    ...
    if (percentageDelta >= 5){
        ...
        UICam.shake(400);
        addSparkles(15);
        if(percentageDelta >= 10){
            addRainbowstars2();
            confettiEmitter.explode()
        }else if(percentageDelta >= 7.5){
            addRainbowstars2();
...
//    console.log(mapArray[xPos,yPos)._isChecked)
    if (mapArray[xPos,yPos]==baseColor && !mapArray[xPos,yPos)._isChecked)
    {
        checkFillCounter++;
        mapArray[xPos,yPos)._isChecked = true;
...
//            console.log("isChecked? : " + mapArray[xPos][yPos]._isChecked)
            ...
            if(!checkedLeft && mapArray[xPos-1][yPos]==baseColor && mapArray[xPos-1][yPos] != checkColor){
                checkFill(xPos-1,yPos,false,true,"both");
                checkedLeft = true;
            }else if(checkedLeft && mapArray[xPos-1][yPos]==edgeColor){
                checkedLeft = false;
            }

            if(!checkedRight && mapArray[xPos+1][yPos]==baseColor && mapArray[xPos+1][yPos] != checkColor){
                checkFill(xPos+1,yPos,true,false,"both");
                checkedRight = true;
            }else if(checkedRight && mapArray[xPos+1][yPos]==edgeColor){
                checkedRight = false;
            }
...
//    UICam.ignore([rt,rtMask])

//    rtMask = rt.createBitmapMask();

//    if(imageSilh.visible){
//        finishedImage.setMask(rtMask)
//        console.log(finishedImage.mask)
//        console.log("AHSOIDASHOIASHOIDAS")
...
//    map = currentScene.make.tilemap({data: revealMask,tileWidth:1,tileHeight:1});
//    layer2 = map.createLayer(0,drawTileset,0,0).setVisible(false);
...
/*
    if(mapArray[i,k+1]==drawColor){
        while(((mapArray[i,k+1])==drawColor || (mapArray[i,k+1])==edgeColor) && k+1<screenH){
            k++;
        }
    }
*/
...
var j=0;
    ...
    for(i=0;i<screenW;i++){
        ...
        while(j<screenH){
            ...
            currentColor = mapArray[i,j];
            belowColor = mapArray[i,j+1];


            if(currentColor==drawColor){
                if(belowColor == baseColor){
                    k=j+2;
                    while(k<screenH && mapArray[i,k).index==baseColor){
                        k+=2;
                    }
            ...
            j+=2;
*/
/*            if (mapArray[i,j]==drawColor && mapArray[i,j+1]==drawColor){
                k = j+1;
                while(k<screenH && !((mapArray[i,k])==drawColor) && !((mapArray[i,k])==edgeColor) && !((mapArray[i,k])==transColor)){
                    k++;
                }
                if(k<screenH && k > j+1){
                    for(m=j+1;m<k;m++){
            ...
            }else if(j+1<screenH && ((mapArray[i,j+1]==edgeColor) || (mapArray[i,j+1]==drawColor))){
                j++;
            }else{
                j++;
            }*/
...
/*
            if (mapArray[i,j]==edgeColor){
                k = j+1
                while(k<screenH && !((mapArray[i,k])==transColor) && !((mapArray[i,k])==drawColor) && !((mapArray[i,k])==edgeColor)){
                    k++;
                }
                if(k<screenH && k > j+1){
                    for(m=j+1;m<k;m++){
